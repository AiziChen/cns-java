...
public class Cipher {
    ...
    public static int xorCrypt(byte[] data, int passwordSub) {
        ...
        int passLen = Main.globalConfig.getPassword().length();
        int pi = passwordSub;
        for (int dataSub = 0; dataSub < dataLen; dataSub++) {
            pi = (dataSub + passwordSub) % passLen;
            data[dataSub] ^= (byte) Main.globalConfig.getPassword().charAt(pi) | (byte) pi;
        }
        return pi + 1;
    }
    ...
    public static int xorCrypt(ByteBuffer data, int passwordSub) {
        ...
        for (int dataSub = 0; dataSub < dataLen; dataSub++) {
            pi = (dataSub + passwordSub) % passLen;
            byte b = (byte) (data.get(dataSub) ^ ((byte) Main.globalConfig.getPassword().charAt(pi) | (byte) pi));
            data.put(dataSub, b);
        }
        return pi + 1;
    }
